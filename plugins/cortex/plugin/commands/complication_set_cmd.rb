module AresMUSH    
  module Cortex
    class ComplicationSetCmd
      ...
      def check_rating
        return nil if Cortex.can_manage_abilities?(enactor)
        Cortex.check_max_starting_rating(self.die_step, 'max_complication_step')
      end
      
      def handle
        ClassTargetFinder.with_a_character(self.target_name, client, enactor) do |model|
          
          trait_config = Cortex.find_complication_config(self.ability_name)
          if (!trait_config)
            client.emit_failure t('cortex.invalid_ability_name')
            return
          end
          
          complication = Cortex.find_complication(model, self.ability_name)
         
          if (complication && self.die_step == '0')
            complication.delete
          ...
          if (complication)
            complication.update(die_step: self.die_step)
          else
            CortexComplication.create(name: self.ability_name, die_step: self.die_step, character: model)
          end
